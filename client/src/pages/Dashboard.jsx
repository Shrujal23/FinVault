...
import CombinedAssetPanel from '../components/CombinedAssetPanel';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    localStorage.setItem('dashboard.compact', compactMode ? 'true' : 'false');
  }, [compactMode]);
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      setLoading(true);
      const response = await apiRequest('/api/assets?enrich=true', { token });
      const nextAssets = Array.isArray(response?.assets) ? response.assets : [];
  ...
  const summary = useMemo(() => {
    const list = assets ?? [];
    const total = list.reduce((sum, asset) => sum + Number(asset.marketValue || 0), 0);
    const invested = list.reduce((sum, asset) => sum + Number(asset.avgBuyPrice || 0) * Number(asset.quantity || 0), 0);
    const pnl = list.reduce((sum, asset) => sum + Number(asset.pnl || 0), 0);
    ...
    const allocation = list
      ...
      .map((asset) => ({
        ...
        weight: Number(asset.marketValue || 0),
      }));

    return { total, invested, pnl, returnPct, allocation };
  }, [assets]);

  const userName = user?.name || user?.email?.split('@')[0] || 'Investor';
  const portfolioHealth = !assets.length
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Based on total return and current holdings mix.</p>
          </div>
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Weekly check-in helps keep your strategy on track.</p>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/95 dark:bg-slate-900/95 rounded-xl shadow-sm border border-slate-200/80 dark:border-slate-800 overflow-hidden min-w-0">
            <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="text-sm text-slate-500 dark:text-slate-400">{assets.length} assets</span>
            </div>
            <div className="p-4 sm:p-6 min-w-0 overflow-x-auto">
              {loading ? <TableSkeleton /> : (
                <CombinedAssetPanel
                  ...
                  onHover={(symbol) => setHoveredSymbol(symbol)}
                />
              )}
            </div>
          </div>
          ...
          <div id="add-asset-section" ref={addAssetRef} className="bg-white/95 dark:bg-slate-900/95 rounded-xl shadow-sm border border-slate-200/80 dark:border-slate-800 p-4 sm:p-6 min-w-0">
            <button
              type="button"
              onClick={() => setIsAddAssetOpen((prev) => !prev)}
            ...
            {isAddAssetOpen && (
              <div className="mt-6 animate-in slide-in-from-top-2 fade-in duration-300">
                <AssetForm token={token} onSaved={() => setRefreshTrigger((value) => value + 1)} />
              </div>
            )}
          </div>
        </div>
